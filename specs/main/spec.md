...
- **FR-002**: System MUST present all user interface elements within a native desktop application window using Electron framework for cross-platform consistency (Windows/macOS/Linux)
...
- **FR-017**: System MUST provide clear, user-friendly error messages for all failure scenarios (see Success Criteria SC-007 for definition)
- **FR-018**: System MUST be buildable as standalone executables for Windows, macOS, and Linux from a single codebase
- **FR-019**: System MUST only require two external services: Ollama (local AI) and MongoDB (data storage); no other backend services or API keys required
- **FR-020**: System MUST NOT require opening a web browser for any application functionality (all UI is embedded in Electron window)
- **FR-021**: System MUST use external MongoDB connection via MONGO_URI environment variable or persisted configuration (supports local MongoDB or MongoDB Atlas cloud)
- **FR-022**: System MUST use local Ollama instance (via OLLAMA_URL environment variable, default http://localhost:11434)
...
- **SC-001**: Application launches from Electron window and displays main UI within 5 seconds on all three supported operating systems (Windows, macOS, Linux) - measured from executable startup to UI fully rendered
- **SC-002**: First-time user (including reading on-screen instructions) completes MongoDB configuration and sees the main interface within 2 minutes - user-perceived time, includes network latency for MongoDB validation
- **SC-003**: Total user-perceived time from PDF file selection through metadata display is <10 seconds for a typical 5-page ethnobotanical article (assumes GPU-accelerated Ollama, local MongoDB, stable network). Breakdown: file upload <2s, PDF text extraction <3s, Ollama inference <3s, display <2s
- **SC-004**: Extracted metadata displays with >90% accuracy for standard fields (title, author count, species count) compared to manual expert review of ground-truth dataset (50+ articles). Accuracy measured as exact match for title, ±1 for author count, ±1 for species count
- **SC-005**: Users can successfully save, retrieve, and edit previously extracted articles without loss of data - verified via database integrity checks and restore tests
- **SC-006**: Database backup can be exported and the resulting ZIP file contains all articles in a verifiable format - ZIP integrity verified by checksum (SHA256), contents validated against MongoDB schema
- **SC-007**: Application handles failures gracefully with specific behaviors: (a) **Network disconnect** → disable upload/extract, show yellow banner "Sem conexão com MongoDB", allow article browsing from cache; (b) **Ollama unavailable** → disable extraction, show red banner "Ollama não disponível - Inicie o serviço e tente novamente", allow all other features; (c) **MongoDB unavailable post-config** → periodic reconnection attempts (60s interval), show status indicator, queue operations for retry. No application crashes in any scenario
- **SC-008**: All three operating system executables are <200 MB each and can be downloaded and run by end users without installation or manual configuration (except MongoDB connection on first run)
- **SC-009**: 95% of users successfully perform the primary workflow (upload PDF → review extracted metadata → save to database) on first attempt without external help - measured via anonymous usage analytics in production or structured user testing with 20+ participants
- **SC-010**: Application closes cleanly without leaving orphaned processes or database connections when user quits via Electron window close button or Application → Quit menu
