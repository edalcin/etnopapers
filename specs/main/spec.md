...
- **No Extractable Text**: When a PDF contains no extractable text (scanned images without OCR), system displays error message with recovery guidance and prevents further processing until user uploads a different file. [RESOLVED]
...
- **Duplicate Articles**: When a researcher uploads a PDF that appears to duplicate an existing article (same title, authors, year), system displays warning with side-by-side comparison of new extraction vs. existing record, offering user options to cancel upload or merge records. [RESOLVED]
...
- **FR-010a**: System MUST detect when PDF contains no extractable text and display error message with recovery guidance ("This PDF appears to be scanned. Please use the original digital file or apply OCR preprocessing.")
- **FR-010b**: System MUST detect potential duplicate articles (same title, authors, year) and display warning with side-by-side comparison, allowing user to cancel or merge with existing record
...
- **FR-036**: System MUST display informational message about GPU benefits on Home page when OLLAMA is configured but GPU is not detected
...
- **SC-003a**: All record management interactions (sorting, filtering, pagination, search) complete in under 200ms when viewing 100+ records
...
- Q: Data model cardinality constraints (plant species, authors, communities per article)? → A: Unlimited cardinality (no validation limits); AI extraction logic is responsible for reasonable content.
- Q: When PDF has no extractable text, what should system do? → A: Display error message with recovery guidance ("This PDF appears to be scanned. Please use the original digital file or apply OCR preprocessing.") and prevent further processing until user uploads different file.
- Q: Performance target for record management UI (100+ records)? → A: Strict performance - all interactions (sorting, filtering, pagination, search) must complete in under 200ms.
- Q: How should system handle duplicate article uploads? → A: Display warning with side-by-side comparison of new extraction vs. existing record, offering user options to cancel upload or merge records.
- Q: When/where to display GPU benefits information? → A: On Home page when OLLAMA is configured but GPU is not detected; contextual and non-intrusive.
